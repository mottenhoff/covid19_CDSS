...
inst_amc_vumc = [inst['institute_id'] for inst in c.request_institutes() if inst['name'] == 'AUMC - AMC'];
records = c.request_study_records(institute=inst_amc_vumc[0])

# %
records = [r for r in records if r['progress'] < 95 and r['progress'] > 5  and (pd.to_datetime(r['created_on']['date']) < datetime.today() - timedelta(days=3))]
