...
# FIXME: Change to def to adhere to pep8 convention
is_in_columns = lambda var_list, data: [v for v in var_list
                                        if v in data.columns]
goal_name = {'icu_admission': 'ICU admission', 'mortality': 'Mortality',
...
def load_data_api(path_credentials):
    ...
    try:
        with open(os.path.join(path_credentials, 'saveddata.pkl'),
                  'rb') as f:  # Python 3: open(..., 'rb')
            df_study, df_structure, df_report, df_report_structure,\
                df_optiongroup_structure = pickle.load(f)
        print('Loading data from PC... delete saveddata.pkl to force ' +
        ...
        with open(str(os.path.join(path_credentials, 'saveddata.pkl')),
