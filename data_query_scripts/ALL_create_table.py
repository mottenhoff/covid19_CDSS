...
import pandas as pd, numpy as np, seaborn as sbn, pickle, os, time
...
for index,row in study_struct.iterrows(): 
    ...
    if var in study_data:
        ...
        for o in sorted(outcomes[0].unique().tolist()):
            ...
            # results[var][o] = study_data[outcomes==o][var]
            # print(np.mean(results[var][o]))
        print('', file=open("output.txt", "a"))
