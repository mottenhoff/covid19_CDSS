...
if False:
    ...
    with open(str(os.path.join(config['CastorCredentials']['local_private_path'],'objs.pkl')), 'wb') as f:  # Python 3: open(..., 'wb')
        pickle.dump([study_data_orig,study_struct,reports_data,reports_struct,optiongroups_struct], f)
else:
    # Getting back the objects:
    with open(os.path.join(config['CastorCredentials']['local_private_path'],'objs.pkl'),'rb') as f:  # Python 3: open(..., 'rb')
        study_data_orig,study_struct,reports_data,reports_struct,optiongroups_struct = pickle.load(f)
        
study_data_orig.reset_index(inplace=True)
