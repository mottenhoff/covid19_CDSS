...
options_corad = c.request_fieldoptiongroup(
    optiongroup_id=c.field_optiongroup_by_variable_name('corads_admission'))

# %%
print('Total COVID-19 records (AMC + VUmc): '+str(len(ct_perf_values)))
print(' - records: CT data not present: ' +
      str(sum([f is None for f in ct_perf_values])))
print(' - records: CT data present: ' +
      str(sum([f is not None for f in ct_perf_values])))

print(' - CORAD scores available (n=' +
      str(sum([f is not None for f in corad_values]))+'): ')
for o in options_corad['options']:
    print('   ' + str(o['name']) + ': ' +
          str(sum([f == o['value'] for f in corad_values])))
